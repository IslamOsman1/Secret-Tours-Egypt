...
import { LayoutDashboard, Map, MessageSquareText, Images, FileText, Settings, LogOut, Plus, Search, Pencil, Trash2, Eye, UploadCloud, Menu, X } from 'lucide-react';
import { api, getStoredSettings, getStoredTours, SETTINGS_UPDATED_EVENT, TOURS_UPDATED_EVENT } from '../api';

const navItems = [['overview', 'Overview', LayoutDashboard], ['tours', 'Tours', Map], ['inquiries', 'Inquiries', MessageSquareText], ['media', 'Media', Images], ['blog', 'Blog', FileText], ['settings', 'Settings', Settings]];

const homeTestimonialsFallback = [
  { name: '', country: '', text: '' },
  { name: '', country: '', text: '' },
  { name: '', country: '', text: '' },
];
...
const blogFallback = [
  { title: '', date: '', excerpt: '', image: '', ctaLabel: 'Read guide' },
  { title: '', date: '', excerpt: '', image: '', ctaLabel: 'Read guide' },
  { title: '', date: '', excerpt: '', image: '', ctaLabel: 'Read guide' },
];
...
export default function AdminDashboard() {
  ...
  return <div className="admin-shell"><aside className={`admin-sidebar ${menu ? 'show' : ''}`}><div className="admin-brand"><img src="/logo.png"/><button onClick={() => setMenu(false)}><X/></button></div><nav>{navItems.map(([id, label, Icon]) => <button className={tab === id ? 'active' : ''} onClick={() => { setTab(id); setMenu(false); }} key={id}><Icon size={19}/>{label}</button>)}</nav><div className="admin-bottom"><Link to="/" target="_blank"><Eye size={18}/> View website</Link><button onClick={logout}><LogOut size={18}/> Logout</button></div></aside>
    <main className="admin-main"><header className="admin-top"><button className="admin-mobile-menu" onClick={() => setMenu(true)}><Menu/></button><div><h1>{navItems.find(item => item[0] === tab)?.[1]}</h1><p>Secret Tours Egypt content manager</p></div><div className="admin-user"><span>AD</span><div><strong>Administrator</strong><small>Content & operations</small></div></div></header>
      ...
      {tab === 'media' && <Media/>}
      {tab === 'blog' && <Coming title="Blog manager"/>}
      {tab === 'settings' && <SettingsPanel tours={items}/>}
    </main>{modal && <TourModal close={() => setModal(false)} add={tour => { setItems(value => [tour, ...value]); setModal(false); }}/>}</div>;
}

function Overview({ tours, inquiries, setTab }) {
  return <div className="admin-content"><div className="stat-grid"><div><span>Live tours</span><strong>{tours.length}</strong><small>Published experiences</small></div><div><span>New inquiries</span><strong>{inquiries.length || 12}</strong><small>Needs follow-up</small></div><div><span>Languages</span><strong>15</strong><small>Frontend locales</small></div><div><span>Content control</span><strong>Live</strong><small>Pages sync from dashboard</small></div></div><div className="admin-panels"><section><div className="panel-head"><h2>Recent tours</h2><button onClick={() => setTab('tours')}>Manage all</button></div>{tours.slice(0, 5).map(tour => <div className="mini-row" key={tour._id}><img src={tour.image}/><div><strong>{tour.title}</strong><small>{tour.city} • ${tour.price}</small></div><span className="status live">Live</span></div>)}</section><section><div className="panel-head"><h2>Quick actions</h2></div><div className="quick-actions"><button onClick={() => setTab('tours')}><Plus/>Add a new tour</button><button onClick={() => setTab('media')}><UploadCloud/>Upload images</button><button onClick={() => setTab('inquiries')}><MessageSquareText/>Review inquiries</button><button onClick={() => setTab('settings')}><Settings/>Site settings</button></div></section></div></div>;
}
...
function Inquiries({ data }) {
  const rows = data.length ? data : [{ _id: 'd1', name: 'Emma Wilson', email: 'emma@example.com', type: 'tailor', createdAt: new Date().toISOString(), status: 'new', message: '8 days, Cairo + Nile cruise, family of 4.' }, { _id: 'd2', name: 'Marco Rossi', email: 'marco@example.com', type: 'contact', createdAt: new Date().toISOString(), status: 'new', message: 'Looking for a private Luxor day tour.' }];
...
function Coming({ title }) {
  return <div className="admin-content"><div className="empty-admin"><FileText size={44}/><h2>{title}</h2><p>The API structure is ready for this module. Add your CMS fields when real content is available.</p></div></div>;
}

function SettingsPanel({ tours }) {
  ...
  const updateSection = (section, key, value) => setForm(state => ({ ...state, [section]: { ...state[section], [key]: value } }));
  const updateListItem = (section, key, index, field, value) => setForm(state => ({ ...state, [section]: { ...state[section], [key]: state[section][key].map((item, itemIndex) => itemIndex === index ? { ...item, [field]: value } : item) } }));
  const updateListValue = (section, key, index, value) => setForm(state => ({ ...state, [section]: { ...state[section], [key]: state[section][key].map((item, itemIndex) => itemIndex === index ? value : item) } }));
  ...
  const toggleTour = id => setForm(state => {
    ...
    return {
      ...
      countdown: {
        ...state.countdown,
        selectedTourIds: exists ? state.countdown.selectedTourIds.filter(item => item !== id) : [...state.countdown.selectedTourIds, id],
      },
    };
  });
  ...
  return <div className="admin-content"><form className="settings-card" onSubmit={submit}>
    ...
    <div className="field-row"><div><label>Map title</label><input value={form.footerMap.title} onChange={event => updateMap('title', event.target.value)}/></div><div><label>Map embed URL</label><input value={form.footerMap.embedUrl} onChange={event => updateMap('embedUrl', event.target.value)} placeholder="https://www.google.com/maps?q=Luxor%2C%20Egypt&z=13&output=embed"/></div></div>
